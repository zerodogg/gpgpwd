...
Contains the version number of the gpgpwd data format. Must be "3" for the
format specified in this file. If this field is a value other than "3" then the
program should error out. If it is a value lower than "3" it may perform an
upgrade to version 3. If it is higher than "3" the program should error out
